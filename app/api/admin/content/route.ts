...
const VALID_KEYS = ["hero", "stats", "offices", "profile", "contacts", "footer", "social"] as const;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // The homepage is statically generated for speed — this makes the
    // edit show up immediately instead of waiting for the next deploy.
    revalidatePath("/");
    const content = await getSiteContent();
    return NextResponse.json({ content });
  } catch (err) {
